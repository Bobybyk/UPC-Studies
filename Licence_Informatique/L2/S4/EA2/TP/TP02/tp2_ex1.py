...
def fibo_1_adds(n) :
    if n <= 0 : return 0, 0
    if n <= 2 : return 1, 0
    tmp1 = fibo_1_adds(n-1)
    tmp2 = fibo_1_adds(n-2)
    return tmp1[0] + tmp2[0], tmp1[1] + tmp2[1] + 1
...
def nbOfBits(i) :
    if i==0 :
        return 1
    return math.floor(np.log2(i)) +1  # À COMPLÉTER
...
def fibo_1_bits(n) :
    if n <= 0 : return 0, 0
    if n <= 2 : return 1, 0
    res1 = fibo_1_bits(n-1)
    res2 = fibo_1_bits(n-2)
    res = res1[0] + res2[0]
    ops_bits = res1[1] + res2[1] + nbOfBits(res)
    return res, ops_bits
