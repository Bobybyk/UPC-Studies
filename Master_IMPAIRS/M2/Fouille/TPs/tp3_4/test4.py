...
def is_list_of_list_of_int(l):
  ...
  for i in range(len(l)):
    if not isinstance(l[i], list):
      return 'element [%d] is %s which is a %s' % (i, l[i], type(l[i]))
    for j in range(len(l[i])):
      if not isinstance(l[i][j], int):
        return 'element [%d][%d] is %s which is a %s' % (i, j, l[i][j], type(l[i][j]))
  return ''

def is_same_tokenized(a, b):
  # Verify the type of the output.
  if len(a) != 3:
    print('FAILED: Expected a triple, got an object of length %d' % len(a))
    return False
  wa, a0, a1 = a
  wb, b0, b1 = b

  # Verify the types of each output element.
  if not isinstance(wa, dict):
    print('FAILED: Expected a dictionary at first output, got a "%s"' % type(wa))
    return False
  for w, i in wa.items():
    if not isinstance(w, str) or not isinstance(i, int):
      print('FAILED: Expected str:int pairs of key:values in the dictionary, got %s:%s' % (
            type(w), type(i)))
      return False
  for name, l in [('second', a0), ('third', a1)]:
    err = is_list_of_list_of_int(l)
    if not err:
      continue
    print('FAILED: Expected a list of int at %s output, got %s' % (name, err))
    return False
...
score += test.Test(
    ...
    data=[
        ('test3_small',
         (expected_test3_small_words, expected_spams, expected_hams), 12),
        ('test3_spaces',
         ({'example':0, 'with':1, 'multiple':2, 'spaces':3},
          [], [[0, 1, 2, 3]]),
         8)
       ])
...
print('Note: The last functions classify_spam*(..) are NOT scored here: there are merely a couple of examples to help you detect syntax or obvious errors. They will be scored by the teacher, on a hidden "test" dataset. You should tune and evaluate the performance of these functions yourself!')
